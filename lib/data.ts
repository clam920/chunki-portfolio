...
export const projects: {
  ...
  {
    title: "Payroll Anomaly API",
    description: "A backend API for payroll processing that automatically flags anomalous timesheets — excessive hours, duplicate entries, unusually high pay — with an approval workflow requiring a documented override reason, secured behind JWT role-based authorization.",
    what: "ASP.NET Core 8 Web API built with Clean Architecture and CQRS (MediatR), where a pluggable rule-based domain service detects timesheet anomalies on every submission and a review workflow forces a human-reviewed override before a flagged item can be approved. Runs on PostgreSQL with versioned EF Core migrations, and JWT authentication separates Manager and Employee actions via role-based authorization policies.",
    interesting: "Designed the override rule to live in the Application layer rather than a single entity, since it needs cross-aggregate knowledge of both Timesheet and Anomaly — a deliberate DDD boundary decision. Layered FluentValidation (request-boundary checks) on top of the domain entities' own constructor guard clauses (last-line-of-defense invariants) rather than picking one over the other. The integration test suite boots the real DI container and hits real HTTP endpoints — including the JWT login flow — against a genuine, disposable PostgreSQL instance spun up via Testcontainers, catching a class of bugs (DI gaps, EF query translation, auth middleware wiring) that mocked unit tests structurally can't.",
    result: "Shipped a three-tier tested (unit + application + Testcontainers-backed integration tests, 58 passing), Dockerized API with centralized exception handling, structured Serilog logging, and CI running the full suite — including containerized integration tests — on every push via GitHub Actions.",
    tags: ["C#", ".NET 8", "ASP.NET Core", "PostgreSQL", "EF Core", "MediatR", "CQRS", "JWT", "FluentValidation", "Serilog", "Testcontainers", "Docker", "xUnit", "Moq"],
    filters: ["All", "Backend"],
