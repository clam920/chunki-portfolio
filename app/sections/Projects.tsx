        ...
        <div key={animKey} className="grid md:grid-cols-2 gap-6">
          {visible.map((project, i) => (
            <div key={project.title}
              className="filter-enter group relative flex flex-col rounded-2xl border border-slate-100 bg-white overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full"
              ...
              <div className="p-6 flex flex-col flex-1">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-bold text-slate-900 text-lg leading-snug pr-4">{project.title}</h3>
                  <div className="flex gap-3 shrink-0">
                    ...
                    {project.demo && (
                      <a href={project.demo} target="_blank" rel="noopener noreferrer"
                        className="text-slate-400 hover:text-emerald-600 transition-colors text-sm font-medium">
                        Demo ↗
                      </a>
                    )}
                  </div>
                ...
                <div className="flex flex-wrap gap-2 mt-auto">
                  {project.tags.map((tag) => (
                    <span key={tag}
                      className="px-2.5 py-0.5 bg-slate-50 border border-slate-200 text-slate-600 rounded-full text-xs font-medium group-hover:border-emerald-200 group-hover:text-emerald-700 group-hover:bg-emerald-50 transition-colors">
                      {tag}
                    </span>
                  ))}
                </div>
